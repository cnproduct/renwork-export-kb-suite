...
export class KnowledgeBaseEngine {
  ...
  public coldStart(tenantId: string, companyName: string, websiteUrl: string, profileSummary: string, industry: string = 'general'): {
    tenantId: string;
  ...
  public searchCards(query: string, options: {
    tenantId?: string;
  ...
  public getCard(kbId: string): KnowledgeCard | undefined {
    return this.cards.get(kbId);
  }

  public saveCard(card: KnowledgeCard): void {
    card.updated_at = new Date().toISOString();
    this.cards.set(card.kb_id, card);
  }

  public auditGovernance(tenantId: string): {
    totalCards: number;
    ...
    const sensDist: Record<string, number> = {};
    let approvedCount = 0;
    let gapCount = 0;
    ...
    for (const c of tenantCards) {
      ...
      if (c.public_claim_approved) approvedCount++;
      if (c.status === 'pending_supplement' || c.confidence < 0.7) gapCount++;
    }
