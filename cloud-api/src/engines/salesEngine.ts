export class SalesExecutionEngine {
  public qualifyInquiry(rawInquiry: string, senderEmail: string = '', senderCountry: string = ''): {
    ...
    if (hasQty) score += 15; else missing.push('采购数量 / 试单计划');
    if (hasSpec) score += 15; else missing.push('规格 / 材质 / 定制要求');
    if (hasDelivery) score += 10; else missing.push('目标交期 / 目的港口');
    if (hasCert) score += 10;

    let status: 'Go' | 'Nurture' | 'Hold' | 'No-go' = 'Hold';
    if (score >= 75) status = 'Go';
    else if (score >= 50) status = 'Nurture';
    else status = 'Hold';

    let escalation_warning: string | undefined;
  ...
  public generateTieredQuote(productName: string, basePriceUsd: number, moq: number): {
    product_name: string;
    currency: string;
    tiers: {
      tier_name: string;
  ...
  public handleObjection(objectionCategory: string, customerStatement: string): {
    category: string;
    root_cause_hypothesis: string;
    recommended_strategy: string;
    scripts: { zh: string; en: string };
  ...
  public triageAftersales(issueDescription: string): {
