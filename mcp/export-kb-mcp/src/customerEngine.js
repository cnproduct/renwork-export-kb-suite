...
exports.CustomerAssetEngine = void 0;
class CustomerAssetEngine {
    accounts = new Map();
    contacts = new Map();
    ...
    scoreAccount(account, interactions = [], opps = [], contacts = []) {
        ...
        // 1. Intent (25 max)
        let intent = 0;
        if (interactions.some(i => ['inquiry', 'quote_request'].includes(i.signal_type)))
            intent += 10;
        if (interactions.some(i => i.signal_type === 'sample_feedback'))
            intent += 8;
        if (interactions.some(i => i.signal_strength === 'strong'))
            intent += 7;
        intent = Math.min(25, intent);
        // 2. Fit (20 max)
        let fit = 10;
        if (account.icp_fit_level === 'A+')
            fit = 20;
        else if (account.icp_fit_level === 'A')
            fit = 16;
        else if (account.icp_fit_level === 'B')
            fit = 10;
        else
            fit = 4;
        // 3. Power (20 max)
        let power = 0;
        if ((account.total_revenue_usd || 0) > 100000)
            power += 10;
        else if ((account.total_revenue_usd || 0) > 20000)
            power += 6;
        if (account.average_repurchase_cycle_days && account.average_repurchase_cycle_days <= 90)
            power += 10;
        else
            power += 5;
        ...
        if (activeOpps.some(o => ['commercial_negotiation', 'pi_issued'].includes(o.stage)))
            stage += 15;
        else if (activeOpps.some(o => o.stage === 'sample_testing'))
            stage += 10;
        else if (activeOpps.some(o => o.stage === 'quote_sent'))
            stage += 6;
        else if (activeOpps.some(o => o.stage === 'rfq_received'))
            stage += 4;
        // 5. Value (10 max)
        let value = 0;
        if ((account.historical_orders_count || 0) >= 2)
            value += 5;
        if ((account.total_revenue_usd || 0) >= 50000)
            value += 5;
        // 6. Quality (10 max)
        let quality = 0;
        ...
        // 7. Risk penalties
        let risk_penalty = 0;
        if (riskFlags.includes('debt_issue'))
            risk_penalty -= 30;
        if (riskFlags.includes('high_bounce'))
            risk_penalty -= 20;
        if (riskFlags.includes('product_mismatch'))
            risk_penalty -= 20;
        ...
        const dynamic_lists = [];
        if (intent >= 10)
            dynamic_lists.push('today_must_follow');
        if (activeOpps.some(o => o.stage === 'quote_sent'))
            dynamic_lists.push('stalled_after_quote');
        if (activeOpps.some(o => o.stage === 'sample_testing'))
            dynamic_lists.push('sample_unconverted');
        if ((account.historical_orders_count || 0) >= 2)
            dynamic_lists.push('repeat_purchase_warning');
        if (fit >= 16 && !activeOpps.length)
            dynamic_lists.push('high_engagement_no_inquiry');
        if (riskFlags.includes('customs_surge'))
            dynamic_lists.push('sourcing_anomaly');
        if (totalScore >= 65 && activeOpps.length > 0)
            dynamic_lists.push('high_score_neglected');
        return {
    ...
    batchCleanAndDeduplicate(rawRecords) {
...
exports.CustomerAssetEngine = CustomerAssetEngine;
